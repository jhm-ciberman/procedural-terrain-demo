...
public class SimplexNoise
{
    ...
    public float Sample(float x)
    {
        ...
        var t0 = 1.0f - x0 * x0;
        t0 *= t0;
        var n0 = t0 * t0 * _Grad(_perm[i0 & 0xff], x0);

        var t1 = 1.0f - x1 * x1;
        t1 *= t1;
        var n1 = t1 * t1 * _Grad(_perm[i1 & 0xff], x1);
    ...
    public float Sample(float x, float y)
    {
        ...
        {
            t0 *= t0;
            n0 = t0 * t0 * _Grad(_perm[ii + _perm[jj]], x0, y0);
        }
        ...
        {
            t1 *= t1;
            n1 = t1 * t1 * _Grad(_perm[ii + i1 + _perm[jj + j1]], x1, y1);
        }
        ...
        {
            t2 *= t2;
            n2 = t2 * t2 * _Grad(_perm[ii + 1 + _perm[jj + 1]], x2, y2);
        }

        // Add contributions from each corner to get the final noise value.
        // The result is scaled to return values in the interval [-1,1].
        return 25f * (n0 + n1 + n2); // TODO: The scale factor is preliminary!
    }
    ...
    public float Sample(float x, float y, float z)
    {
        ...
        {
            t0 *= t0;
            n0 = t0 * t0 * _Grad(_perm[ii + _perm[jj + _perm[kk]]], x0, y0, z0);
        }
        ...
        {
            t1 *= t1;
            n1 = t1 * t1 * _Grad(_perm[ii + i1 + _perm[jj + j1 + _perm[kk + k1]]], x1, y1, z1);
        }
        ...
        {
            t2 *= t2;
            n2 = t2 * t2 * _Grad(_perm[ii + i2 + _perm[jj + j2 + _perm[kk + k2]]], x2, y2, z2);
        }
        ...
        {
            t3 *= t3;
            n3 = t3 * t3 * _Grad(_perm[ii + 1 + _perm[jj + 1 + _perm[kk + 1]]], x3, y3, z3);
        }
    ...
    private static int _FastFloor(float x)
    {
        return (x > 0) ? ((int)x) : (((int)x) - 1);
    }

    private static int _Mod(int x, int m)
    {
        var a = x % m;
        return a < 0 ? a + m : a;
    }
